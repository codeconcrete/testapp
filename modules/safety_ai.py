...
def generate_risk_assessment(api_key, task_name, location, risk_factors, risk_context_manual, protectors, safety_equip, tools, materials, ref_vocab_text, ref_risks_text):
    ...
    try:
        ...
        4. [핵심 중요사항: 대책 내용의 길이 제한]
           - **각각의 대책 내용 1줄은 반드시 최대 35자를 넘지 않도록 극도로 간결하게 핵심만 작성하세요.**
           - 이유: PDF 출력 시 글자가 조금만 길어져도 두 줄로 넘어가버려 표 공간을 비효율적으로 차지합니다. 
           - **나쁜 예 (35자 초과):** "- 안전모 턱끈 체결 및 전체식 안전벨트 등 개인 보호구 착용 상태를 철저히 점검함" (X)
           - **좋은 예 (35자 이내):** "- 개인 보호구 착용 상태 철저히 점검" (O)
           - 불필요한 조사나 서술어("~를 확인한다", "~를 철저히 점검함")를 과감히 생략하고 명사형이나 명료한 단문으로 요약하세요.
        5. [중요] **위험성 평가 점수(빈도×강도)를 보수적으로 산정하세요.**
        ...
        6. 반드시 JSON 포맷으로만 출력하세요. (Markdown 코드 블록 없이 순수 JSON만 출력)
        7. [중요] 위 '현장 표준 용어'에 있는 표현을 우선적으로 사용하세요. 현장 실무 용어를 그대로 쓰세요.
        8. [중요] **반드시 최소 12개 이상, 최대 20개 이하의 위험요인 항목**을 생성하세요.
           - '작업준비': 3~4개
           - '본작업: (세부단계1)': 3~5개
           - '본작업: (세부단계2)': 3~5개 (필요시 세부단계 추가)
           - '작업종료/정리': 2~3개
        9. [중요] **위험요인은 반드시 1가지 위험만 기술하세요.** 여러 위험을 "및", "또는"으로 묶지 마세요.
